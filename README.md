...
| `architecture-ddd-lite-fullstack` | 开始编写或审查 Java / React / Vue / Flutter 业务代码前 | 强制 DDD-lite 分层、Feature 模块化、单向依赖与原子能力沉淀；要求代码结构清晰、易维护、低耦合、高内聚，禁止 UI / Controller 直接承载业务逻辑；**Service 业务动作扩展强制阈值（T1-T6）**——已有 ≥3 个业务动作 public 方法 / 新动作引入新状态机 / 新下游 / 新事务边界 / 新前置校验 / 新方法 > 80 行，命中任一必须建独立子 service / UseCase，原 service 不允许新增第 N 个业务动作 public 方法 |
