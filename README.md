...
| `hooks/event-log.js` | **warn 档 hook 命中事件本地登记**（Node 跨平台，best-effort）：被 `check-sql-ddl-readiness` / `check-backend-kg-readiness` 命中时调用，追加一行 `{ts,user,host,plugin,hook,rule,mode,tool,file}` 到 `~/.kai-toolbox/hook-events.jsonl`，供统计"规则命中频率 / 升不升 block"。**只写本地、绝不碰网络；全程 try/catch，登记失败不影响放行/拦截**。同步到 `\\IT01` 共享 + 周报统计在 `yoooni-daily-plugin`（本插件不感知）。详见 `docs/design/hook-event-logging.md` | 接入新 warn hook 或调整事件 schema 时 |
