...
面向 AI 原生项目的跨项目工程治理插件。当前版本：**3.4.0**。它只保留团队层面稳定复用的分析、设计、架构、编码、文档和交付能力；项目特有的路由、脚手架、架构 lint、编码例外和系统拓扑由项目自身维护。
...
3.3.0 引入意图、授权与风险的独立判断，区分咨询交付和已授权实施；3.4.0 继续保留 21 个 Skill，并收敛重复文档及旧位置门禁。
