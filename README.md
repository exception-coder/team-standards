...
| `architecture-ddd-lite-fullstack` | 开始编写或审查 Java / React / Vue / Flutter 业务代码前 | 强制 DDD-lite 分层、Feature 模块化、单向依赖与原子能力沉淀；要求代码结构清晰、易维护、低耦合、高内聚，禁止 UI / Controller 直接承载业务逻辑；**Service 业务动作扩展铁律——新业务分支 = 新子 service，无阈值无豁免**：扩展既有 service 时只要新增的是新业务分支（独立业务动作 / 新状态机 / 新下游 / 新事务 / 新前置校验任一），必须建独立子 service / UseCase / Handler，原 service 不新增 public 方法（仅允许 1 行 delegate），不论原 service 行数；唯一例外是同一分支的参数化变种 |
