...
4.4.0 将概设/详设维护接入普通开发任务：Agent 自动发现、补建、绑定与同步，不要求用户手工初始化。已有正文原位复用，准备记录保护初始脏文件与并行绑定；未启用 OpenSpec 的项目支持本地验收引用。见[自动执行流程](plugins/team-standards/skills/change-readiness/references/automatic-design-maintenance.md)及[实际试点与验证](docs/maintenance/automatic-design-release.md)。
...
面向 AI 原生项目的跨项目工程治理插件。当前版本：**4.4.0**。它只保留团队层面稳定复用的分析、设计、架构、编码、文档和交付能力；项目特有的路由、脚手架、架构 lint、编码例外和系统拓扑由项目自身维护。
...
管理看板围绕目标与结果、差距与风险、原因分解和行动明细；目标、历史趋势和归因必须有证据，缺数据时选择摘要或明细。可直接提出“根据采购对象及状态设计领导看板”，也可调用 `$business-visualization-advisor`。这是 Agent 设计能力，不是已部署的自动看板服务。统计总览或看板优化时，编码前必须明确受众、发现目标、后续行动和首屏问题，建立“目的—指标—图表—行动”映射及逐项候选选图清单；没有改变图表需提前说明理由，交付区分信息架构、图表化与交互改造，并提供视觉证据或缺口。完成前按业务目的反向验证；不能仅以换标题、数字卡或布局配色宣称看板优化完成，至少证明一项决策、洞察、比较、定位或行动能力的实际改善。
