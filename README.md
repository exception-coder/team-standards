...
| `coding-standards-common` | 编写/修改任何源码语言（Java / TS / JS / Dart / Python / Kotlin / Go 等）前 | 跨语言通用编码铁律 7 条 + 注释三档：命名表意 / 函数原子（80 行硬阈值）/ 层次分明 / 零魔法值（强制枚举 DB 字段值与协议码）/ 注释三档（类 1-3 行 + 方法 1-2 行 + 核心块 1 行）/ 异常不静默 / DRY rule of 3 |
