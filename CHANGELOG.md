...
- 浏览器验收覆盖高屏、矮屏、窄屏、动态窗口变化及长短内容；同步规则入口与视觉检查表，不新增形式化 CSS 检查器。

## [3.4.0] - 2026-09-12

- 设计和现状说明复用唯一载体，取消强制 coding/ai-ref、个人 Phase-A/B 索引和固定图数；日报默认 Git、按需持久化。
- 退役个人文档位置门禁，写入分发减为 7 项，保留旧调用兼容入口。
- 治理策略/检查器版本 2：查重与交接可选，不适用视图无需另写正文，支持原始验证结果文件引用；审阅、范围、任务与输入指纹仍检查。
- 旧绑定同会话迁移保留基线、范围和重试预算，不自动升级旧 PASS；同步规则、流程、全景与源码说明。

## [3.3.0] - 2026-09-12

- 统一探查、修复、演进、接入意图，授权与操作风险独立判断，按问题对象选择主 Skill。
- 分离咨询交付与实施阶段，已有授权连续有效，不因阶段或风险档位重复审批。
- 明确纯重构回归、接入检查边界及 Hook 非沙箱；同步入口、Skill 与路由场景，不新增路由器或门禁。

## [3.2.0] - 2026-09-12

- 共享契约路径归一，新增默认预览的主副本单向同步，拒绝覆盖消费者未提交改动。
- 根 README 版本与 Skill 数量从 manifest/实际目录更新；发布预检检查元数据漂移。
- 新增 test:fast/test:full，保留 npm test 全量发现和现有门禁；临时输出统一使用忽略目录。
- 移除架构 Skill 描述中的 Dart 残留，保留 21 个独立意图入口。

## [3.1.0] - 2026-09-12

- 本地验证按变更影响分级，保留 Forge 与 CI 完整门禁；Skill 指令不视为纯说明。
- 精简 Markdown、提交和工作日志入口，详细格式按需加载；提交正文保留，正常收尾不重复展示。
- 工作日志统一按主题批处理，S 档默认免写；取消按文件数和对话轮次机械累加工时。
...
- 同步解除架构规则中的 dartdoc 要求；保留 Dart 精确数值与 TODO 规则，不批量转换已有源码注释。

## [2.8.0] - 2026-09-12

- Bug 处理保留诊断证据、最小修复和回归要求；取消简单问题的独立报告、固定图表、目录与章节要求。
- 复杂问题复用已有共享载体；文档位置门禁放行项目 docs/bug/，其他路径继续使用原规则。
- 工作日志和代码定位不再依赖先生成 Bug 文档；保留 bug-doc-required 调用名及既有引用。

## [2.7.0] - 2026-09-12

- AI 原生作业将受影响文档与实现同次交付；统一 README、规则入口和索引同步职责。
- 默认完成后自动提交扩展至适用规范的套件及业务项目，只提交本次范围；push 授权保持独立。
- 移除普通业务提交必须反复确认和全量暂存的旧指引，保留明确禁止、验证失败与混合改动边界。
- 修正英文 README 的 Skill 数量和入口文档中的 Hook 测试路径；本次未新增自动执行 Hook。
...
**补齐 SQL 标识符的目标数据库兼容性约束。**

- Oracle 未验证长标识符支持时按 30 bytes 上限设计表/列别名、CTE、索引和约束等标识符。
- SQL 短别名与完整领域字段显式映射,新增或修改别名时补长度断言或目标 Oracle 解析测试。
- 登记 `ORA-00972` 真实违规与反模式案例,防止 AI 再次照搬完整领域字段名生成 SQL 别名。

## [1.54.0] - 2026-08-16

**增强 `frontend-excellence` 的人工设计判断，增加可恢复状态与反通用生成式 UI 视觉复核。**

- 视觉规则从泛化的“避免模板感”细化为层级先于容器、克制的形状/材质默认值和有目的的视觉手段。
- 新增工作流状态参考，要求空、错、失效、无权限和校验状态保留上下文并提供可信下一步。
- 新增视觉复核清单，专项检查大白卡、机械居中、巨大状态图标、过度圆角、装饰性渐变和全局阴影。
- 完成门禁要求在首轮真实渲染及重大视觉调整后复核，记录有意保留的例外。

## [1.53.0] - 2026-08-13

**新增 `frontend-excellence` Skill，把前端交付标准从“代码可运行”提升为“设计系统化且经过真实浏览器验收”。**

- 在既有 `architecture-ddd-lite-fullstack` 之后增加 Web 前端专项链路，保持分层规则单一来源。
- 新增架构与状态、设计系统与美学、质量门禁三份渐进参考。
- Figma、截图与 ImageGen 均可作为输入；Figma 不成为强制依赖。
- 强制覆盖响应式、可访问性、完整交互状态，并验证移动与桌面关键视口。

## [1.52.0] - 2026-08-13

**修复小改与自动收尾绕过提交正文生成的问题，并把正文升级为可机械验证的改动闭环。**

- `git-commit-standards` 改为任何提交前都触发，改动大小只决定是否追加五步重审。
- 提交正文统一要求 `【改动】`、`【原因】`、`【结果】` 三段，分别陈述代码动作、决策依据和解决结果。
- 消息生成器与 Bash Hook 共用同一正文校验器，缺少任一段时阻断并给出明确提示。

## [1.51.0] - 2026-08-11

**运行载荷变化必须伴随版本递增，版本同步从“值相等”升级为“相对基线确实变大”。**

- CI 根据 Skill、Hook、命令、Agent、App 和 MCP 载荷白名单识别运行时变化，版本未递增时阻断。
- README、设计文档、测试、benchmark 和纯发布脚本不触发插件发版。
- Git 提交规范明确 Plugin 与 MCP 引擎、知识内容的不同刷新协议。
...
- 新增 `domain-spec-mining-required`，在遗留系统规格缺失或冲突时编排 Graphify、DDL、反向索引、运行证据与 `project-domain-knowledge` 挖掘 CLI。
- 建立 observed、inferred、candidate、confirmed 四级证据边界，自动晋升上限为 draft。

### Changed

- 状态/关联类需求在设计前必须给出对象终态、旧关联解除、失败/幂等、数据库断言和下一业务动作。
- 知识图谱链路从正向事实与反向影响扩展为“事实 → 对象规格候选 → 影响面 → 设计”。
- 候选评审绑定证据版本；证据漂移后旧评审自动过期，禁止继续晋升。

## [1.48.0] - 2026-08-07

**插件可靠性第二轮：真实安装、共享契约、安全自检、性能预算和发布 dry-run 形成闭环。**

### Added
- 三个插件增加隔离 Codex CLI 安装与 Hook 自动发现冒烟测试。
- 两个写入插件增加 v1 Golden Fixtures、SHA-256 完整性门禁和匿名本地 Hook 耗时指标。
- 新增工作区镜像防漂移检查与 `release-team-tools.mjs`，生成三插件 `tar.gz`、文件清单和 SHA-256，不执行提交或发布。

### Changed
- CI 增加独立相对性能预算；普通单元测试不承担易抖动的绝对耗时门禁。
- personal marketplace 条目补齐安装与认证 policy，release dry-run 强制检查清单、版本和策略完整性。

## [1.47.0] - 2026-08-07

**写入 Hook 完成 Codex `apply_patch` 兼容、并发分发和隐私安全默认值加固。**

### Added
- 新增统一 `Change[]` 输入适配器，覆盖 Claude `Write/Edit/MultiEdit` 与 Codex 新增、更新、删除、移动、多文件补丁。
- 新增单进程 Worker 分发器及契约、端到端、隐私回归测试。

### Changed
- 五个写入规则统一检查补丁中的全部文件；无命中基准平均耗时从 540.4 ms 降到 175.8 ms（-67.5%）。
- 提示词信号默认仅保存疑问和纠正，落盘前脱敏并截断为 1000 字符，不再保存目录、会话、用户和主机字段。
- Codex 清单改用标准 `hooks/hooks.json` 自动发现，并通过插件结构校验。
