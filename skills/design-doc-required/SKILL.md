...
| **完整** | `template.md` | 超出轻量范围的方案/接口设计：新增表/字段/对外契约、复杂事务/分布式锁、状态机调整、跨服务风险等 | **需要**，按 `coding-template.md` 生成 |
...
> 3. 参考模板填写内容（至少完成以下章节）：
>    - 第 1 节：目标与边界
>    - 第 2 节：接口 / 入口契约（如涉及接口变更）
>    - 第 3 节：核心流程（必填 1 张 Mermaid 核心图）
>    - 第 4 节：核心业务规则
>    - 第 5 节：编码落点
>    - 第 7 节：风险与待确认
> 4. 草稿确认后告知我文件路径，我将读取后开始实现；终版是否上传到项目 `docs/` 由用户自行决定
...
|------|----------------------|---------------------------|
| 目标、边界、设计结论 | 保留 | 摘要引用 |
| 接口/入口契约 | 保留核心字段 | 展开到请求示例与方法入口 |
| 核心流程图 | 保留 1 张核心图 | 不重复 |
| 核心业务规则 | 保留 | 保留并可展开实现约束 |
| 编码落点 + 一句话职责 | 保留 | 展开方法签名和操作步骤 |
| 数据/依赖变化 | 只写本次变化 | 展开字段级、调用级细节 |
| 风险与验证要点 | 保留 | 转换为测试/异常处理要点 |

**原则：设计文档回答"为什么这样做、核心逻辑是什么、风险在哪里、代码大概落哪"；编码文档回答"每个方法怎么写"。**
...
- 变更记录 → 精简为一行摘要
- 第 1 节目标与边界 → 精简为设计结论和不做什么
- 第 2 节接口/入口契约 → 提取入口契约和请求示例
- 第 3 节核心流程 → 提取关键步骤和分支
- 第 4 节核心业务规则 → 原文提取
- 第 5 节编码落点 → 提取**全路径**填入「涉及类清单」，补充方法签名和职责
- 第 6 节数据与依赖变更 → 提取本次变化的关键字段、事务、下游调用
- 第 7/8 节风险与验证 → 提取异常处理、边界条件和测试要点
...
|---------|---------|---------------|------|
| **接口自身流程图** | 第 3 节（核心流程） | `flowchart TD` | 单接口、单业务动作、退款/下单/查询等自身流程 |
| **库表读写时序图** | 第 3 节（核心流程） | `sequenceDiagram` | 读写表顺序、字段含义、事务边界是核心 |
| **状态流转图** | 第 3 节或第 4 节 | `stateDiagram-v2` | 状态迁移是本次核心风险，且用流程图表达不清 |
| **模块/依赖关系图** | 第 6 节（数据与依赖变更） | `graph TD` | 新增跨系统依赖或复杂模块交互，文字/表格说不清风险 |
...
- [ ] 至少包含 1 张核心图（接口自身流程图 / 库表读写时序图二选一）
...
- [ ] 数据结构、下游依赖、缓存、消息、锁、事务只写本次变化，不复写项目全集资料
