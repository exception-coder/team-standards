...
>    - 第 4 节：业务流程设计
>    - 第 5 节：接口设计（如涉及接口变更）
>      - 新增接口时，**第 5.5 节请求示例为必填**
>      - 新增接口时，**必须定义独立的 Request/Response DTO，禁止复用已有接口的 DTO**
>    - 第 8 节：核心业务规则
...
- 第 5 节接口设计 → 提取方法签名（须含全类名）；若存在 5.5 请求示例则原文提取
- 第 8 节核心业务规则 → 原文提取
...
| "新接口参数结构一样，可以复用旧 DTO" | 新增接口必须定义独立 DTO；复用会导致两个接口耦合，未来任何一方变更都可能破坏另一方 |
