...
| 5 | 核心业务流程 | 从 Service 调用链推断 | 关键业务流程，标注 Service、状态变迁、事件。**每步附证据来源** |
| 6 | 关键约束与扩展点 | Service + AOP + 配置 | 事务边界、幂等点、鉴权入口、状态守卫、外部失败补偿、兼容性约束 |

**C 组：接口与交互（跨服务图谱拼接区）**

| # | 维度 | 数据来源 | 说明 |
|---|------|---------|------|
| 7 | 对外暴露接口 | Controller 层 | Method + URL + 入参 + 出参，按业务域分组 |
| 8 | 对外调用服务 | FeignClient / RestTemplate / HTTP Client | **本服务调用了谁**：服务名 + 接口 + 用途 |
| 9 | 事件与消息契约 | MQ Listener + Publisher | **发布什么事件、消费什么事件**：topic + payload + 触发条件 |
...
| 11 | 配置概要 | application*.yml | **仅列业务相关配置**（功能开关、限额、超时等），脱敏处理 |
