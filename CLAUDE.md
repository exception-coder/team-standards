...
| **初始化全局 Design System / 从项目提取 Design Profile / 绑定或 fork 风格** | `design-system-bootstrap`（Registry 固定在 `$HOME/.design-registry`；个人 Skill 官方目录为 `$HOME/.agents/skills`） |
| **有意义的 UI 实现或修改需要继承既有视觉 DNA** | `design-system-guardian`（先解析 Registry / Binding / Profile；成熟区域 Conservative，新区域 Controlled Exploration） |
| **用户明确表达 UI 审美反馈**（太 AI、舒服、图 2 好、圆角太大、布局割裂等） | `design-observer`（只记录 scoped Preference Evidence，不直接改规则） |
| **同类设计 Evidence 重复出现或要求归纳偏好** | `design-pattern-miner`（更新 Candidate 强弱、冲突和晋升建议，重大晋升需人工确认） |
| **新页面 / 大型 UI 重构 / 新组件体系 / Navigation / 响应式重做 / Release 视觉验收** | `design-reviewer`（基于 Profile、References 与 Rubric 识别 Drift，不另造设计语言） |
...
| **即将 Write/Edit 任何描述后端表关系/ER/SQL/状态扭转/业务流程→表 CRUD 的 .md（无论路径，包括 `ai-docs/`、`work-log/`、`scenarios/`）**；用户问表关系、字段来源、业务怎么查、SQL 怎么写/完善、退款/账单/流水/分摊怎么算等表关系/SQL/状态/原子能力问题；编写或评审统计、筛选、分页、批量遍历、循环数据访问，尤其出现无界聚合、应用层过滤分页、全候选扫描、N+1；AI 完成后端代码调查发现可复用事实或 SQL 查询逻辑；后端接口/服务开发前涉及表读写、SQL、状态判定、订单/退款/支付等业务逻辑；存在 `docs/knowledge-graph/backend/`；要求生成/更新后端知识图谱、全景 ER、SQL 归档或查询逻辑索引；**或同一会话同一技术主题（含子进程编排 / 并发 / 性能 / 资源争夺 / 外部依赖等非业务技术陷阱）用户反复疑问 ≥3 轮、出现回归性措辞（"为什么...还" / "怎么又..." / "上次说..." / "现在又卡了"）、修复后 ≥2 轮验证追问** | `backend-knowledge-graph-required` |
...
| **探索初步 PRD、生成初始化规格、价值分析或工时评估，且证据可能跨主项目与重构、迁移、依赖或集成项目** | `planning-evidence-discovery`（先解析项目证据范围，再按关系查询并复用持久化轨迹） |
...
> 29 个 skill 按使用阶段分 8 组，遇到具体任务先按组检索而不是遍历全表。每个 skill 详细描述见下方「Skill 索引」。

| 阶段 / 类别 | 包含 Skill | 用途速记 |
|---|---|---|
| **① 方案 / 需求分析（动手前最先触发）** | `solution-review-required` / `planning-evidence-discovery` / `design-doc-required` / `bug-doc-required` / `business-logic-orientation` | 把"用户想做什么"和"实际怎么做"分离；跨项目取证后产出设计文档 / bug 文档 / 现状梳理文档 |
...
|-----------|------|---------|--------|
| `frontend-excellence` | `skills/frontend-excellence/` | 创建、重做或显著提升生产级 Web 前端时触发；在既有 Feature 分层之上，强制读取项目设计系统与数据模式，建立明确视觉方向和语义令牌，拒绝无依据的 Card 化、机械居中和装饰性效果，覆盖响应式、可访问性、可恢复的加载/空/错/失效/无权限状态，并通过真实浏览器在移动与桌面视口完成视觉复核和交互验收；Figma、截图与 ImageGen 均为可选输入，不允许把可运行 mock、单文件 HTML 或编译成功当作最终交付 | 前端、Web UI、React、Vue、Next.js、仪表盘、门户、落地页、组件库、设计系统、视觉美学、反 AI UI、状态恢复、响应式、可访问性、Figma 转代码、截图转代码、浏览器验收、视觉回归 |
| `design-system-bootstrap` | `skills/design-system-bootstrap/` | 初始化用户级 Design Registry、从现有项目提取 Profile、创建 inherit/reference/independent Binding 或 fork Profile；只提取 Observed Evidence，不在首次初始化时脑补完整 Design System | 初始化 Design System、Design Registry、Design Profile、design.config.json、绑定风格、fork 风格、`.agents/skills` |
| `design-system-guardian` | `skills/design-system-guardian/` | 有意义的 UI 工作前解析 Registry、Binding、Profile、References 与现有实现；成熟区域按 Reuse→Extend→Compose→Invent，未知区域受控探索；按改动范围决定验证成本 | UI 治理、Design Drift、视觉 DNA、Conservative、Exploration、复用设计、反 AI UI |
| `design-observer` | `skills/design-observer/` | 用户明确表达审美接受、拒绝或比较时，将反馈写成最小 scope 的 Preference Evidence；不修改 Design Rule | 太 AI、舒服、图 2 好、圆角太大、布局割裂、Preference Evidence |
| `design-pattern-miner` | `skills/design-pattern-miner/` | 重复 Evidence 出现时归纳 Candidate、增强/削弱候选、登记冲突、提出弃用或晋升建议；Profile Evidence 不直升 Global Core | Pattern Mining、Candidate、Evidence 重复、规则晋升、冲突、弃用 |
| `design-reviewer` | `skills/design-reviewer/` | 新页面、大型 UI 工作或里程碑时，按 Profile 与 Visual Eval Rubric 检查视觉和工程 Drift；小改不强制完整视觉循环 | 视觉评审、UI Review、Design Drift、Rubric、响应式验收、Release Eval |
...
| `git-commit-standards` | `skills/git-commit-standards/` | **任何提交都先触发**，包含小改与自动收尾；统一生成类型标题、`【改动】【原因】【结果】` 三段中文 body 和真实 Author；改动大小只决定 `check-git-commit-skill.js` 是否追加五步重审，不再决定是否进入 Skill；Hook 仅作宿主支持时的机械兜底；team-standards 自动 push 仍受宿主命令授权策略约束 | 提交、commit、git、分支、push、授权、hook、提交正文、自动收尾 |
...
| `java-coding-standards` | `skills/java-coding-standards/` | **阿里黄山版 Java 独占条款**（通用 7 条见 coding-standards-common）：Java 命名补充、格式、Javadoc、OOP、集合、并发、SLF4J、关系库 SQL 规范与安全；SQL 标识符须匹配目标数据库兼容级别，Oracle 未验证长标识符支持时按 30 bytes 上限设计并补映射/解析回归；同时禁止应用层过滤后分页、循环逐条数据访问和加载全量明细做统计 | Java、Javadoc、Integer 比较、BigDecimal、StringBuilder、HashMap 容量、SLF4J、SimpleDateFormat、ThreadLocal、关系库、SQL、Oracle、ORA-00972、标识符长度、别名、应用层分页、N+1、全量统计、执行计划 |
...
| `backend-knowledge-graph-required` | `skills/backend-knowledge-graph-required/` | 后端单服务知识图谱 + 项目级技术难点图谱 + DDL 基线 + **高风险查询性能门禁**：沉淀表、SQL、状态与原子能力；写 SQL 前核对真实 DDL；无界聚合、应用层过滤分页、全候选批次遍历、循环 DAO/Mapper/Repository 和 N+1 在落码前必须告警，估算候选规模与最坏 SQL 次数，比较过滤下推、集合化、预计算或有界异步方案，并取得执行计划、调用次数或真实计时；无可信环境时标记“性能未验证” | 后端、SQL、统计SQL、全量统计、全表扫描、派生筛选、应用层分页、批次遍历、循环查询、N+1、执行计划、DBMS_XPLAN、EXPLAIN ANALYZE、DDL 基线、表关系、状态、原子能力、性能风险 |
...
| `planning-evidence-discovery` | `skills/planning-evidence-discovery/` | PRD 初始化规格、价值分析和工时评估的跨项目证据编排：强制先调用平台 resolver 获取主项目、关系、角色和真实路径；按 `REFACTORS`、`MIGRATES_FROM`、`DEPENDS_ON`、`INTEGRATES_WITH` 动态选择领域知识、Graphify、DDL、路由、源码和跨项目拓扑；新旧事实分栏，真实工具结果持久化为 `planning-evidence-trace-v2`，由服务端完成性门禁驱动最多三轮补查 | 规格探索、PRD 探索、初始化规格、价值分析、工时评估、跨项目证据、重构来源、迁移来源、证据轨迹、planning evidence |
...
| `init-project-docs` | `skills/init-project-docs/` | 初始化当前代码项目知识：优先用 Graphify 建立可再生事实层，按“纯代码 AST → 宿主 Graphify skill → Claude CLI/已配置 CLI 后端 → 经确认的语义降级 → 确定性扫描”路由；禁止因缺 API Key 静默执行 `--code-only`；以 Git `source_commit` 标记基线；**输出统一到用户目录知识库** `{USER_DOCUMENTS}/ai-docs/{project}/`。与入职初始化互斥：出现“项目文档+知识图谱/Graphify”时本 skill 优先，禁止检查 SVN 或创建凭据文件 | 初始化当前项目文档、生成知识图谱、Graphify、分析项目能力、生成项目概要、init project docs、knowledge graph |
...
| 文件 | 所属 Skill | 用途 |
|------|-----------|------|
| `skills/solution-review-required/SKILL.md` | solution-review-required | 用户提出具体方案并要求实施前的目标/方案分离、现有代码质量评估、风险识别与更优建议规则，含生产优先原则（Production First 八条 + Production/Exploration 模式判定 + 毕业检查） |
...
| `skills/planning-evidence-discovery/references/tool-contracts.md` | planning-evidence-discovery | Forge 项目范围解析、统一证据查询、关系语义、状态枚举与完成性门禁契约 |
| `skills/planning-evidence-discovery/references/planning-evidence-trace-v2.schema.json` | planning-evidence-discovery | 跨项目规划证据轨迹的机器校验 JSON Schema |
...
| `skills/backend-knowledge-graph-required/references/query-performance-gate.md` | backend-knowledge-graph-required | 无界统计、派生筛选、应用层分页、批次全扫和 N+1 的风险回显、优化顺序与运行证据门禁 |
...
| `skills/frontend-excellence/references/interaction-and-states.md` | frontend-excellence | 工作流状态、恢复路径、控件状态、导航与状态文案规则 |
| `skills/frontend-excellence/references/visual-review-checklist.md` | frontend-excellence | 真实渲染后的视觉层级与通用生成式 UI 模式复核清单 |
| `skills/frontend-excellence/references/quality-gates.md` | frontend-excellence | 响应式、可访问性、性能、浏览器验证与交付证据门禁 |
| `skills/design-system-bootstrap/references/resolver.md` | design-system-bootstrap | Global Registry、项目 Binding、Profile、References 与 Guardian Mode 的统一解析规则 |
| `skills/design-system-bootstrap/references/registry.schema.json` | design-system-bootstrap | Global Registry Manifest JSON Schema |
| `skills/design-system-bootstrap/references/profile.schema.json` | design-system-bootstrap | Design Profile JSON Schema |
| `skills/design-system-bootstrap/references/design-config.schema.json` | design-system-bootstrap | 项目 `design.config.json` Binding Schema |
| `skills/design-system-bootstrap/references/evidence.schema.json` | design-system-bootstrap | Preference Evidence JSONL 单条记录 Schema |
| `skills/design-system-bootstrap/references/reference-metadata-template.md` | design-system-bootstrap | Approved / Rejected Reference Metadata 模板 |
| `skills/design-system-bootstrap/assets/registry-template/` | design-system-bootstrap | `$HOME/.design-registry` 最小初始化模板与 Visual Eval Rubric |
| `skills/design-system-bootstrap/assets/profile-template/` | design-system-bootstrap | 新 Profile 的 Observed-first 初始化模板 |
...
| `hooks/check-architecture-boundaries.js` | architecture-ddd-lite-fullstack（兜底） | **新增模块边界机械检查** — `tools/tool-*/pom.xml` 新增另一个 `tool-*` 依赖、前端 feature 新增对另一 feature 内部路径的 import 默认 exit 2；通过 `public-api` 的跨 feature 引用放行；源码预计超过 500 行且继续新增有效内容时仅软提醒；`TEAM_STANDARDS_ARCH_BOUNDARY_HOOK=warn\|off` 可降级或关闭，`TEAM_STANDARDS_GIANT_FILE_LINES` 可调阈值 |
...
| `hooks/check-sql-ddl-readiness.js` | backend-knowledge-graph-required（兜底） | 写 SQL、Mapper、DAO 或 Repository 前检查本会话是否读取 DDL 基线；默认 warn，`TEAM_STANDARDS_SQL_DDL_HOOK=block\|off` 可调 |
| `hooks/check-query-performance-risk.js` | backend-knowledge-graph-required（兜底） | 对新增文本中的无界聚合、循环数据访问和集合过滤后内存分页做启发式软提醒；始终不阻断，`TEAM_STANDARDS_SQL_PERF_HOOK=off` 可关闭 |
