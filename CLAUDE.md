...
| **即将 Write/Edit 任何描述后端表关系/ER/状态扭转/业务流程→表 CRUD 的 .md（无论路径，包括 `ai-docs/`、`work-log/`、`scenarios/`）**；**用户在会话中问 "X 与 Y 是 1:1 还是 1:N"、"改这个动哪些表"、"字段从哪来"、"退款/账单/流水/分摊怎么算"、"是新建快照还是引用原表"等表关系/状态/原子能力问询**；AI 完成后端代码调查发现 ≥1 条可复用事实（含以另一项目为调查对象的场景，图谱归属永远 = 被调查项目而非 cwd）；后端接口/服务开发前涉及表读写、状态判定、订单/退款/支付等业务逻辑；存在 `docs/knowledge-graph/backend/`；要求生成/更新后端知识图谱 | `backend-knowledge-graph-required` |
...
| `backend-knowledge-graph-required` | `skills/backend-knowledge-graph-required/` | 后端单服务知识图谱：按项目沉淀领域能力、原子能力、流程、表、表关系、枚举、状态判定、API、外部依赖与代码坐标；**v1.18.7 起强化主动触发**——即将 Write/Edit 任何描述后端表关系/ER/状态扭转/业务流程的 .md（无论路径，含 `ai-docs/`、`work-log/`、`scenarios/`）必须先经本 skill；用户问"X 与 Y 是 1:1 还是 1:N"/"改这个动哪些表"/"字段从哪来"/"分摊怎么算"等表关系/状态问询必须自动触发并候选沉淀；AI 完成后端调查发现可复用事实必须自动追加候选池；**调查对象路由**：图谱归属永远 = 被调查的后端单服务项目，与当前 cwd 无关（cwd=A 调查 B 云端时图谱归 B），不再被误判为跨项目转给 `cross-project-locator`；**渐进式三层骨架**：Tier 1 起步只要 `00_index.md`+`scenarios/{场景}.md`+`_candidates.md` 即可投入使用，Tier 2/3 按规模扩展；候选池路径精简为 `ai-docs/{被调查项目}/knowledge-graph/_candidates.md`（按项目而非按日期累积） | 后端、接口开发、知识图谱、单服务、表逻辑、表关系、订单状态、部分退、退款判定、原子能力、DAO、SQL、枚举、状态流转、候选沉淀、API、Service、写前拦截、问询触发、调查对象路由、被调查项目归属、cwd 无关、渐进式三层、Tier 1/2/3、scenarios 场景小卡 |
