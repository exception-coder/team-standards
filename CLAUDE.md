...
| `init-project-docs` | `skills/init-project-docs/` | 初始化当前代码项目知识：优先用 Graphify 建立可再生事实层，再生成项目说明与待确认领域知识；以 Git `source_commit` 标记基线；**输出统一到用户目录知识库** `{USER_DOCUMENTS}/ai-docs/{project}/`。与入职初始化互斥：出现“项目文档+知识图谱/Graphify”时本 skill 优先，禁止检查 SVN 或创建凭据文件 | 初始化当前项目文档、生成知识图谱、Graphify、分析项目能力、生成项目概要、init project docs、knowledge graph |
