...
**兜底规则：** 若 Codex 即将对源码文件（`.java`、`.dart`、`.ts`、`.py`、`.kt` 等）执行 Edit/Write 操作，必须先按 S/M/L 表完成设计档位检查。只有完整满足 S 档硬条件时，才可记录“极简跳过”并不生成设计文档；不得凭主观判断以“任务简单”为由跳过。Bug 修复、纯重构等其它例外仍由 `design-doc-required` 内部判断。
