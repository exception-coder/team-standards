...
| `doc-index-required` | `skills/doc-index-required/` | 写文档前先判定输出路径（团队共享 `docs/` vs 个人临时 `.ai-docs/`）；团队共享文档读取总索引与子目录索引、分析内容边界、半自动更新索引；个人临时输出写入本地忽略目录且不更新索引 | 文档、docs、写文档、索引、重复内容、输出路径、个人文档、临时文档、ai-docs |
...
| `bugfix-coding-style` | `skills/bugfix-coding-style/` | 联调期代码改动两类规范：A 类（修改/替换/删除）禁止直接删原代码，必须先 `//` 注释保留 + 追加新代码，带 `[DEPRECATED YYYY-MM-DD]` 标记；B 类（纯新增缺漏代码）必须加 `[ADDED YYYY-MM-DD]` 头注释 + 缺漏原因 + 对齐依据；仅由用户明确指令清理（A 类删整段，B 类仅删头注释保留代码）；适用于轻量修订流水分支，不适用于新功能开发 | 联调期、bug修复、对齐云端、删冗余、补缺漏、注释保留、DEPRECATED、ADDED、bugfix style |
