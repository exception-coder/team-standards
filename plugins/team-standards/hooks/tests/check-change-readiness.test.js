// =============================================================
// 端到端测试 check-change-readiness.js
...
const HOOK = path.resolve(__dirname, '..', 'check-change-readiness.js');
...
test('放行：TEAM_STANDARDS_CHANGE_READINESS_HOOK=off 一次性绕过', () => {
...
test('兼容：旧 TEAM_STANDARDS_DESIGN_DOC_HOOK=off 仍可绕过', () => {
  const root = mkTmpProject('legacy-bypass');
  try {
    const { code } = runHook({
      tool_name: 'Write',
      tool_input: { file_path: path.join(root, 'lib', 'main.dart'), content: '' },
...
test('阻断：OpenSpec 只有空模板或 change artifacts 不完整', () => {
  const root = mkTmpProject('openspec-incomplete');
  try {
    const changeRoot = path.join(root, 'openspec', 'changes', 'add-feature');
    ...
    assert.equal(code, 2);
    assert.match(stderr, /OpenSpec|设计依据/);
  } finally {
    fs.rmSync(root, { recursive: true, force: true });
  }
});
...
test('阻断：源码改动且无设计文档', () => {
  ...
  try {
    ...
    assert.equal(code, 2);
    assert.match(stderr, /未检测到可用设计依据|未找到/);
    assert.match(stderr, /change-readiness/);
  } finally {
    fs.rmSync(root, { recursive: true, force: true });
  }
});
