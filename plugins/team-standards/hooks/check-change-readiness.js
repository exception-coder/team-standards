#!/usr/bin/env node
// =============================================================
// PreToolUse hook: 写源码前检查变更是否已有可用设计依据
//
...
若本次确为极简改动且确认跳过，可设置环境变量绕过本次会话：
  PowerShell: $env:TEAM_STANDARDS_CHANGE_READINESS_HOOK = 'off'
  bash/zsh:   export TEAM_STANDARDS_CHANGE_READINESS_HOOK=off

模板：参考 skills/change-readiness/{lightweight-template.md, template.md}
`);
    process.exit(2);
  }
  process.exit(0);
