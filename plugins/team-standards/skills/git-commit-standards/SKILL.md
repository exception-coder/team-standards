...
**提交前主动填写（所有提交强制）**：不得先执行 `git commit` 再等待 Hook 报错。必须先根据本轮会话填写标题和中文正文，调用 `scripts/build-commit-message.js` 读取 Git 作者并生成消息文件，随后统一使用 `git commit -F <消息文件>`。Hook 是遗漏兜底，不是正文生成器。
...
```bash
node "<skill-dir>/scripts/build-commit-message.js" --repo "<repo>" --title "type(scope): 标题" --body "中文变更说明"
git commit -F "<脚本输出的绝对路径>"
```

标题和正文由 AI 在脚本调用前根据会话上下文写好；脚本只做输入校验、读取 Git Author 和安全写文件，不会根据 diff 猜测语义。正文包含多段或特殊字符时，仍作为一个 `--body` 参数传入，禁止退回只有标题的 `git commit -m`。
