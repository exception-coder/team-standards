...
description: Use when the user asks to commit code or generate a commit message AND the staged diff is non-trivial (>2 files, >30 net lines, or contains new/renamed/deleted files). For trivial commits (≤2 files, ≤30 lines, only modifications to existing files) skip the five-step workflow, but still write the complete title + Chinese body + Author message required by this skill — `hooks/check-git-commit-skill.js` only controls workflow entry, while `hooks/check-commit-no-ai-signature.js` enforces message structure for every commit. Compose the message from the current session's edit intent first; only re-read diffs to cover gaps. Automatic stage/commit/push applies ONLY to the team-standards plugin source repository, never to business projects that merely install this plugin.
---
...
- **小改放行**：≤ 2 文件 ∧ insertions+deletions ≤ 30 ∧ 全部为 `M`（仅修改现有文件，无 `A`/`R`/`D`）→ 跳过本 skill 的五步重流程，但提交消息仍必须使用“标题 + 中文正文 + Author”完整格式。阈值可用环境变量 `TEAM_STANDARDS_TRIVIAL_FILES` / `TEAM_STANDARDS_TRIVIAL_LINES` 调整。
...
- **提交结构与署名机械兜底**：无论大改小改、是否进本 skill，`hooks/check-commit-no-ai-signature.js` 都会在 `git commit` 落盘前校验标题、中文正文和真实 `Author`，任一缺失即 `exit 2`；同时继续拦截 `Co-Authored-By` 指向 AI / `noreply@anthropic.com` / 机器人 emoji / `Generated by <AI>`。默认 block；`TEAM_STANDARDS_COMMIT_MESSAGE_HOOK=warn|off` 可降级，旧变量 `TEAM_STANDARDS_AI_SIGNATURE_HOOK` 继续兼容。
