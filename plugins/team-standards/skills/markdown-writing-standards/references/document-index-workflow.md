---
name: document-index-workflow-reference
description: "Internal document deduplication and index-registration rules loaded by markdown-writing-standards."
---
...
{USER_DOCUMENTS}/ai-docs/{project}/
  ...
  orientation/                       ← 重构/迁移前现状梳理
    ...
    {模块名}/
      {模块名}-现状梳理.md
      {模块名}-ai-ref.md
  knowledge-graph/                   ← 历史项目证据目录，仅兼容读取
...
| `knowledge-graph/` | `00_index.md`（自有结构） | ✅ 必须，但读 `00_index.md` 不读 `INDEX.md` | ✅ 由 backend-evidence 自身负责 |
...
**调用规则：**
- 调用方（如 `change-readiness`、`bug-doc-required`、`business-logic-orientation`）写文档前必须调用 Phase-A，写完后必须调用 Phase-B
...
- **历史 `knowledge-graph/` 只读兼容**：不得再由 `backend-evidence` 建立全量代码索引；当前实现和影响查询交给 Graphify，稳定业务语义进入 domain knowledge
...
| [knowledge-graph/](knowledge-graph/00_index.md) | 历史项目证据 | 仅兼容读取；新代码事实使用 Graphify |
...
| `knowledge-graph` | `knowledge-graph/00_index.md` | 由 `backend-evidence` 接管，本 skill 仅做存在性校验 |
...
文档写作完成后，生成并直接写入当前作业必要的索引更新；这属于文档同步授权，不重复请求确认。用户明确要求预审时，先准备具体更新内容再请求确认。
...
| "继续维护一套手工代码知识图谱" | **错**。当前实现和反向影响使用 Graphify；历史 `knowledge-graph/` 只兼容读取 |
| “文档索引会自动完成” | **错**。调用方必须显式执行写前查重和写后登记 |
