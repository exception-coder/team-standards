...
description: "当用户明确要求批量清理存量代码中的版本标记、历史叙事、废话、死代码或其他违规注释时使用；不用于普通改码时的顺手清理。"
---
...
| `bug-doc-required` 修复模式 | 修改同一段代码时按 common 清理无用标记；Bug 修复不扩大成全仓清理，本 Skill 仅处理用户主动发起的批量任务 |
