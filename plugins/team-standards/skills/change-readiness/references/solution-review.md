---
name: solution-review-reference
description: "Internal proposal-review rules loaded by change-readiness; not a standalone Skill."
---
...
不确定时默认按 Production 执行，不为模式标签本身请求确认。此处 Exploration 表示明确的一次性原型，不等于意图分类中的只读探查 Explore。
...
5. **给出更优建议**
   ...
   - 暂缓方案：先补齐影响证据，仅对仍未决的选择或授权缺口确认
   - 反惯性建议：当现有代码质量差时，提出“先抽模型/规则，再落代码”的路径

6. **形成执行决策**
   - 低风险且更优路径明确：直接按推荐方案实施
   - 多方案权衡明显：先简短说明推荐理由，再实施推荐方案
   - 超出已有授权、尚未授权的破坏性操作或存在未决业务选择：准备具体方案后仅补齐所缺确认；高风险和团队规则变更本身不要求重复审批
...
纯方案咨询只交付审视结果，不因命中本参考就创建 change 或实施。已有明确实施授权且方案合适时，可以简化为一句：
...
- 用户只提出需求目标，没有给出具体方案：先走 `change-readiness`
- 用户已经提出具体方案或要求照某个方案实施：先走本 skill，再走 `change-readiness`
- 本 skill 负责“方案是否值得做、怎么做更好”，`change-readiness` 负责“正式设计文档和编码门禁”
...
| “给用户很多方案让他自己选” | 默认给出明确推荐；只有无法消除的歧义、未决选择或授权缺口才补充确认 |
