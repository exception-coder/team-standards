...
1. proposal 明确谁使用、为什么做、目标及可衡量成功标准；缺依据的效果指标写待确认，不能自动补收益。
2. delta specs 以稳定功能编号关联 Requirement/Scenario；同一功能允许多个 Requirement。正常、异常、权限、并发和恢复按实际影响覆盖，不为每项机械新建测试。
3. design 说明本次差异、取舍和功能正文映射。模块概设按八章说明全貌，详设按功能九项组织；共享模型/权限/部署/迁移集中引用，不复制整份当前设计到 change。
4. 设计准出时具名审阅业务可读性、合理性及技术正确性，记录身份、范围、问题和处理。Agent 自审如实标明，不冒充领导或产品确认。
5. 实施切片通过实际验证后同步对应功能全景、当前正文、主规格及证据引用；规划中/实现中与已验证清晰分开，已上线另外引用发布记录。检查新增、更新和删除/替代功能的实际正文与关联。
6. 归档再核对整个 change 的同步完整性、旧描述和消费者引用。未完成目标留活动 change；内容覆盖检查不替代内容审阅或官方归档。

机器配置及内容审阅快照沿用[内容治理协议](design-content-governance.md)，不新增一套生命周期引擎。
