...
查重同时读取相关主规格、活动 changes，必要时追溯归档。主规格已覆盖的需求先核对实现缺失或偏离，不重复添加同名 Requirement。判断依据写回 proposal 的查重章节，恢复会话优先沿用原绑定。
...
| 实施与验证 | tasks/validation：实际结果、偏差及证据 | 当前交付范围通过验证，规格与设计一致 |
...
概设与详设遵循 [设计输出契约](design-output-contract.md)；需要绑定、检查或记录证据时读取 [治理检查器协议](governance-checker.md)。记录使用现有 tasks/validation，机器文件只做结构化索引，不再维护平行正文。

## 评审、变更与交接

需求负责人确认业务目标与验收含义，设计/实施负责人维护方案，验证负责人提供结果，交付负责人核对同步与归档。允许兼任，但 Agent 自审必须标为 Agent，不得伪装独立审阅或人工批准。

专家意见记录来源、范围、问题、依据、接受/拒绝/延期结论、责任及复核结果。没有来源时标为待核实建议。改变业务行为先回写 proposal/delta specs，再更新设计、任务和验证；仅改变技术实现从 design 开始；实现或证据缺口落入 tasks/validation，不删除验收条件来掩盖失败。
...
交接在 tasks/validation 维护一处：change 与任务范围、仓库/分支/基线、当前阶段及准出缺口、有效规格/设计/验证引用、阻塞责任与恢复条件、下一项可执行任务和未执行的同步/归档动作。恢复时核验原绑定与证据新鲜度，不因为更换会话而另立需求。
...
4. 需要在归档前提前合并 delta specs 时调用 `openspec-sync-specs`；不要自行编写 spec 合并器。
   同步前重新读取主规格并核对基线；同一 Requirement 的并行 changes 先消除冲突，重复同步应无内容漂移。长期设计按已接受实现原位晋升并更新索引，不能把 proposal/design/tasks 全文拼接进主规格。
