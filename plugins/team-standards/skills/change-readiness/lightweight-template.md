...
> 本模板是按需选用的提纲；只在唯一设计载体中补充相关内容，不强制独立文件、空章节、版本流水或固定图数。
...
>
> **职责定位**：本文档说明接口设计意图，当前库表访问关系由 Graphify 和源码核实 ——
> - 编码前：作为接口原子能力的设计意图，确认要动哪些表、按什么顺序、过滤规则是什么、失败行为如何
> - 编码后：同步与设计不符的决策，不维护另一份库表行为索引
>
...
> 用文件链接定位关键入口。
> - 仅保留定位所需的文件和符号，不维护易失效行号清单
> - 复杂调用链按需查询图谱并核实源码，不全量转抄
...
涉及统计、筛选、分页、批量遍历或循环数据访问时，追加以下检查表；普通主键查询可省略：

| 候选规模 / 硬上限 | 批次大小 | 单候选查询数 | 最坏 SQL 次数 | 调用频率 | 优化方案 | 执行计划 / 实测 |
|---|---|---|---|---|---|---|
| `{典型值 / 峰值}` | `{条}` | `{次}` | `{公式}` | `{在线 / 定时 / 离线}` | `{下推 / 集合化 / 预计算 / 有界扫描}` | `{证据或性能未验证}` |
