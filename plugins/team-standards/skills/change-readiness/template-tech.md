...
> - 接口字段级契约复用已有权威来源，缺少时在同一设计中补充相关章节
...
> **【按需·Mermaid】** 一张 `graph TD` 或 `graph LR`，展示模块边界与依赖方向。
>
...
> 设计文档只确认落点和职责，不展开方法伪代码。必要约束留在当前设计章节，具体方法实现留在代码，不另建编码摘要。
>
...
> 接口字段级契约引用已有权威来源；没有时在本节补齐，不另建副本。
...
涉及统计、筛选、分页、批量遍历或循环数据访问时，必须补充候选规模、最坏 SQL 调用数、调用频率、执行计划摘要、真实计时和保护边界；无法取得可信运行证据时标记“性能未验证”，不得表述为生产可用。
