仅在用户要求或项目明确需要 Registry/Profile 时使用。
...
2. If absent, copy the minimal [registry template](../assets/registry-template) and validate it with [registry.schema.json](registry.schema.json).
...
6. Validate profiles, bindings, and evidence with the schemas in `references/`.
   - Use [reference-metadata-template.md](reference-metadata-template.md) for approved and rejected visual references.
7. Report created paths, evidence sources, unresolved choices, and validation results.

## 偏好学习模式

- 用户明确表达接受、拒绝或比较某个 UI 选择时，读取 [evidence-capture.md](evidence-capture.md)，只记录最小作用域的 Preference Evidence，不直接改设计规则。
- 同类 Evidence 重复出现、互相冲突或用户要求归纳偏好时，读取 [pattern-mining.md](pattern-mining.md)，更新候选强弱、冲突与晋升建议。
- 单次反馈不得直接升级为 Profile 或 Global Core；重大晋升必须人工确认。

## Resolver and storage contract

Use [resolver.md](resolver.md). Skills may be installed from a plugin, repository `.agents/skills`, or the official personal directory `$HOME/.agents/skills`; evolving registry data always stays outside Skill directories at `$HOME/.design-registry`.
