...
本流程只在用户明确要求沉淀现状文档、项目没有可复用知识入口，或重构需要长期基线时加载。普通现状理解不得执行本参考中的双文档产出。
...
flowchart TD
    ...
    B --> C["调用 markdown-writing-standards 写前阶段\n读取索引 + 分析内容边界"]
    C --> D["判断前端/后端/全栈"]
    D --> E["深度阅读源代码\n按调用链逐层展开"]
    E --> F["按标准模板生成梳理文档\n场景级 3 图 + 核心代码 + 知识图谱"]
    F --> G["生成 AI 速查索引\n紧凑结构，纯索引+表格"]
...
> 现状梳理默认写入用户目录知识库 `{USER_DOCUMENTS}/ai-docs/{project}/orientation/`，由 `markdown-writing-standards` 的写前查重与写后登记管控；用户明确指定项目路径或要求上传终版时才进入 `{项目根}/docs/orientation/`。

```
{ROOT}/orientation/                              ← {ROOT} = {USER_DOCUMENTS}/ai-docs/{project} 或 {项目根}/docs
  INDEX.md                                       ← 子索引，由 markdown-writing-standards 写后阶段维护
  {业务模块名}/
...
| Skill | 何时调用 |
|-------|---------|
| `markdown-writing-standards` | 创建文档前读索引与分析边界；文档写完后更新索引 |
| `markdown-writing-standards` | 生成 Mermaid 图表时，遵循语法规范 |
| `change-readiness` | 梳理完成后，若进入重构设计阶段，须调用此 skill 创建重构设计文档 |
| `change-readiness` 代码定位模式 | 重构设计文档确认后，开始写代码前调用 |
