---
name: domain-spec-mining-required
description: "Use before designing or coding business changes that alter orders, inventory, labels, audits, cancellations, returns, transfers, allocations, or other object states/associations when the existing domain specification is missing, incomplete, or contradicted. Also use when asked to mine a domain model, discover a state machine or invariant, generate an executable domain specification from legacy code/data/logs, or verify business closure beyond the current API."
---
...
把代码、DDL、反向索引和运行记录转成**有证据的候选规格**，防止 AI 以“接口成功”代替业务终态和下一动作。Graphify 负责静态事实，本门禁编排 `project-domain-knowledge` 的 `spec-mining` CLI；不重复实现挖掘算法。
...
2. **定义对象边界**
   ...
   - 字段语义不清时回到 DDL、反向索引和读写点，不凭字段名猜。
3. **补证据缺口**
   - 静态事实：复用 Graphify `graph.json`、DDL 基线和 reverse index。
