...
>
> **最后更新：2026-04-27 v14**
> 变更摘要：新增 `solution-review-required`，当用户提出具体方案并要求实施时，先分离真实目标与候选方案，识别风险、替代方案和更优建议，再进入设计文档或实施。`doc-index-required` 的个人/临时 AI 输出默认路径从项目内 `.ai-docs/` 调整为用户 Documents 下的 `ai-docs/{project}/`，避免影响团队工作区。
> 上一版：v13（2026-04-27）。
> 再上一版：v12（2026-04-27）；v11（2026-04-27）；v10（2026-04-27）；v9（2026-04-26）；v8（2026-04-25）；v7（2026-04-22）；v6.2 `bug-doc-required` 调整目录结构为三级；v6.1 新增 Step 0 知识图谱预热。
> 历史版本：`docs/skill-flow-20260427-v13.md`（v13）、`docs/skill-flow-20260427-v12.md`（v12）、`docs/skill-flow-20260427-v11.md`（v11）、`docs/skill-flow-20260427-v10.md`（v10）、`docs/skill-flow-20260427-v9.md`（v9）、`docs/skill-flow-20260425-v8.md`（v8）、`docs/skill-flow-20260422-v7.md`（v7）、`docs/skill-flow-20260416-v6.md`（v6）、`docs/skill-flow-20260410-v5.1.md`（v5.1）、`docs/skill-flow-20260404-v4.md`（v4）、`docs/skill-flow-20260403-v3.md`（v3）、`docs/skill-flow-20260402-v2.md`（v2）
...
| `doc-index-required` | team-standards | **(辅助)** 创建任何 Markdown 文档前先判定输出路径；团队共享文档写 `docs/` 并更新索引，个人/临时 AI 输出写用户 Documents 下的 `ai-docs/{project}/` 且不更新索引 |
...
flowchart TD
    ...
    %% 入口路径
    E1 --> SRR --> BRAIN --> PLAN --> CTX --> DOP --> DDR
    E2 --> SRR --> CTX --> DOP --> DDR
    E3 --> DEBUG --> CTX --> DOP --> BDR
    E4 --> SRR --> BLO --> CTX --> DOP --> DDR
    E5 --> IPD
...
flowchart TD
    ...
    CTX --> DOP["doc-index-required\n先判定输出路径\n团队共享 docs / 用户目录 ai-docs"]
...
| 需要单独调用 doc-index-required 吗? | 创建任何 Markdown 文档前都要先应用它的“输出路径判定”。若最终写入 `docs/`，再执行 Phase-A/Phase-B 更新 INDEX；若写入用户目录个人/临时路径，不更新 INDEX。 |
| 个人分析草稿应该写到哪里? | 默认写到用户 Documents 下的 `ai-docs/{project}/{agent}/{YYYY-MM-DD}/`；Windows 为 `%USERPROFILE%\Documents\ai-docs\...`，macOS/Linux 为 `~/Documents/ai-docs/...`，无 Documents 时兜底 `~/ai-docs/...`。 |
| 正式设计文档还能写 `docs/design/` 吗? | 可以。已确认要作为团队共享编码依据的正式设计文档仍写 `docs/design/`，草稿和临时分析不写入该目录。 |
