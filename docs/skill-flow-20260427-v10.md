...
>
> **最后更新：2026-04-27 v10**
> 变更摘要：新增 `architecture-ddd-lite-fullstack`，作为编码前默认架构规则，要求在 `pre-implementation-code-orientation` 之后、第一行业务源码改动之前完成 DDD-lite 分层判断；覆盖 Java Spring、React、Vue、Flutter，强制 Feature 模块化、单向依赖、原子能力沉淀，并拦截 UI / Controller 直接承载业务逻辑或访问 DB / HTTP。同步更新 Skill 总览、主调用关系图、功能开发链路、Bug 修复链路和常见困惑。
> 上一版：v9（2026-04-26）。
> 再上一版：v8（2026-04-25）；v7（2026-04-22）；v6.2 `bug-doc-required` 调整目录结构为三级；v6.1 新增 Step 0 知识图谱预热。
> 历史版本：`docs/skill-flow-20260427-v9.md`（v9）、`docs/skill-flow-20260425-v8.md`（v8）、`docs/skill-flow-20260422-v7.md`（v7）、`docs/skill-flow-20260416-v6.md`（v6）、`docs/skill-flow-20260410-v5.1.md`（v5.1）、`docs/skill-flow-20260404-v4.md`（v4）、`docs/skill-flow-20260403-v3.md`（v3）、`docs/skill-flow-20260402-v2.md`（v2）
