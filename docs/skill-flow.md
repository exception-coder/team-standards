...
>
> **最后更新:2026-05-11 v21.1**
> 变更摘要 v21.1:**规则方向反转 + 清理冗余文件** —— 删除 `docs/skill-flow-{YYYYMMDD}-v{N}.md` 全部 18 份历史快照(v2~v21),并把 CLAUDE.md / AGENTS.md / README.md 维护规则中"链路结构变化时同时创建版本快照"反转为"skill-flow.md 直接更新,历史由 git log 承担,不再创建文件式快照"。理由:skill-flow.md 已在 git 仓库管理,任意历史版本均可通过 `git show <sha>:docs/skill-flow.md` 还原,文件快照与 git 历史完全冗余;同样的原则在 v18.2 已应用于设计文档(current.md + git log),本次将原则补齐到 skill-flow.md。"较大 / 轻微"判断保留,但只用于决定是否同时调 dev-log 记录决策背景,不再触发文件创建。配套:`.claude-plugin/plugin.json` 与 `marketplace.json` 1.24.0 → 1.24.1;`.codex-plugin/plugin.json` 1.16.0 → 1.16.1。
> 变更摘要 v21:**新增两个 skill** 用于支撑「PRD + 项目源码 → 工时 + 开发方案 + 影响面分析」AI Agent 的输入侧知识规范 —— ① `glossary-required`:业务术语会话级强制登记,精简五栏(中文名 / 英文标识 / 一行定义 / 同义词 / 关联代码坐标);PRD 与对话中出现 ≥1 个未登记业务领域名词、用户与 AI 同义词错位(「退货」vs「退款」)、AI 完成代码调查发现新术语映射时自动候选追加;候选池 `{USER_DOCUMENTS}/ai-docs/{project}/glossary/_candidates.md`、正式版 `docs/knowledge-graph/glossary.md`;与 `init-project-docs/templates/07_glossary.md` 分工 — init 负责批量初始化完整版,本 skill 负责日常对话级增量补登;不收录通用编程概念(归 backend-knowledge-graph 技术难点)和跨项目同名异叫法(归 cross-project-locator)。② `reverse-index-required`:反向影响索引强制维护(4 类) — states.md(枚举/状态值 → 判断点 + 业务语义 + 新增态时是否需补判断)、fields.md(字段 → 读 / 写点 + 同步报文是否包含 + 改名风险)、events.md(同步事件 → 订阅场景 + 报文字段 + 消费方 + 接入清单)、apis.md(API → 调用方 + 入参 / 出参变更协同清单);冷启动用 `node hooks/scan-reverse-index.js`(新增 Node 跨平台扫描器,支持 Java / Dart / TS 枚举 + EnumName.VALUE 引用 + SQL 字面量候选)产出 states.md 初版,fields/events/apis 仅生成存根需人工填充;增量维护规则:每次变更枚举 / 字段 / 事件 / API 同回合必须回写反向索引,严禁延迟到下次会话;候选池 `{USER_DOCUMENTS}/ai-docs/{project}/knowledge-graph/reverse-index/`、正式版 `docs/knowledge-graph/reverse-index/`;与 `backend-knowledge-graph-required` 互补(正向 vs 反向)、与 `cross-project-locator` 边界明确(单服务内 vs 跨项目调用方);V1 已知限制:不识别 case 裸值 / 反射 / 配置文件,需人工补到候选区。配套:`hooks/scan-reverse-index.js` 新增手工运行扫描器(默认未注册到 hooks.json);`.claude-plugin/plugin.json` 与 `marketplace.json` 1.23.1 → 1.24.0;CLAUDE.md / README.md 主动触发表 + Skill 索引 + 辅助资源同步追加。该变更属于**新增 2 个 skill 节点**(链路结构变化)。
> 变更摘要 v20.5:`coding-standards-common` §5.0 **规则方向反转**——从「默认 = 沟通语言 + 存量文件已是另一种语言则保持一致避免中英混用」改为「**沟通语言一票否决，无存量文件豁免**」。原版给 AI 留了"原文件是英文所以新增也写英文"的合法退路，实际效果是中文沟通的会话里 AI 大量沿袭存量英文注释，规则被退路架空。新版核心：① 中文沟通 → 新增 / 修改注释一律中文，英文沟通 → 一律英文，**不沿袭存量**；② 短期内同一文件中英混杂可接受（待后续重构统一），**不要为了避免混杂而违背沟通语言**；③ 唯一能覆盖默认的合法路径，是用户在本会话内明确要求特定语言（如「这个项目按公司规范全英文」）；④ 自检清单第六项同步改为「不沿袭存量文件语言」。配套：`.claude-plugin/plugin.json` 与 `marketplace.json` 1.23.0 → 1.23.1；`.codex-plugin/plugin.json` 1.15.0 → 1.15.1；CLAUDE.md / README.md skill 索引覆盖范围同步追加"无存量文件豁免"。该变更属于既有 skill 的**规则方向反转**（rule direction reversal，原退路被废止），链路节点结构未变，按"轻微"处理未单独创建快照。
> 变更摘要 v20.4：`korepos-backend-service` 新增 **DTO 字段类型强制约束** —— Step 2/3 通用章节追加「禁止 `Object?` / `dynamic` / `Map<String, dynamic>` / `List<Map<String, dynamic>>` 容忍多形态」一节：所有 Request / Response / Data 块字段必须声明唯一确定类型（`String` / `int` / `double` / `bool` / 强类型子 DTO / 强类型 List），多分支（短路 vs Rust FFI、本地组装 vs 云端透传）形态融合一律在 service 层归一后再写入 DTO；嵌套结构必须建强类型子 DTO，可空用 `Type?` 而非 `Object?`；唯一例外是字段语义本身就是"任意 JSON"的扩展位/raw payload，需在 dartdoc 写明。典型反例 `calculate_refund_price_response.dart` 顶层金额 `taxAmount` / `serviceFeeAmount` / `tipAmount` / `refundAmount` 用 `Object?` + 嵌套块用 `Map<String, dynamic>` 必须改造。配套：`.claude-plugin/plugin.json` 与 `marketplace.json` 1.22.0 → 1.22.1；CLAUDE.md skill 索引覆盖范围与关键词同步追加。该变更属于既有 skill 的约束补强（同 Step 2/3 通用章节内的兄弟规则），链路节点结构未变，按"轻微"处理未单独创建快照。
> 变更摘要 v20.3：`korepos-backend-service` 强化 wire DTO 注解约束并加 hook 兜底 —— ① **DTO 注解强制约束**：`features/{module}/common/models/(request|response)/*.dart` 下 wire DTO 必须用 `@JsonSerializable(explicitToJson: true)`，禁止 `@freezed`；理由：项目无全局 `explicit_to_json`，freezed 默认 `toJson` 不递归会让 service 内部 Map 风格读 / 改嵌套子项时 cast 失败爆 `_TypeError`（已在 korepos-refund 算价接口踩坑修复一次）；wire DTO 不需要 freezed 的不可变 / `copyWith` / `==` 三件套；唯一例外是 sealed class / union types / pattern matching，需在文件头加 `// FREEZED-EXCEPTION:` 标记。② **新增 hook 兜底**：`hooks/check-dto-annotation.js`（跨平台 Node 脚本，PreToolUse Write/Edit/MultiEdit 默认启用）拦截违规——`@freezed`（无 `FREEZED-EXCEPTION` 标记）和裸 `@JsonSerializable()`（缺 `explicitToJson: true`）均 `exit 2` 阻断，stderr 输出错误信息回灌 AI；环境变量 `TEAM_STANDARDS_DTO_HOOK=off` 临时禁用。③ **SKILL.md 改动**：Step 2 上方新增「Step 2/3 通用：DTO 注解强制约束」章节（含「为什么」「对比表」「强制写法」「唯一例外」「Hook 兜底」），Step 2/3 模板代码 `@JsonSerializable()` → `@JsonSerializable(explicitToJson: true)`。④ **plugin 版本**：`.claude-plugin/plugin.json` 与 `marketplace.json` 1.21.3 → 1.22.0；`.codex-plugin/plugin.json` 1.13.3 → 1.14.0。CLAUDE.md / AGENTS.md skill 索引同步追加约束描述与关键词；README.md hooks 表追加新脚本行。该变更属于触发条件强化 + 新 hook 兜底（机制层补丁），链路节点结构未变，按"轻微"处理未单独创建快照。
...
> 上一版:v20.5(2026-05-09,轻微规则补充未单独创建快照);v20.4(2026-05-08,轻微规则补充未单独创建快照);v20.3(2026-05-08,轻微规则补充未单独创建快照);v20.2(2026-05-07,轻微规则补充未单独创建快照);v20.1(2026-05-06,轻微规则补充未单独创建快照);v20(2026-05-05);v19.11(2026-05-05,轻微规则补充未单独创建快照);v19.10(2026-05-04,轻微规则补充未单独创建快照);v19.9(2026-05-03,轻微规则补充未单独创建快照)；v19.8（2026-05-03，轻微规则补充未单独创建快照）；v19.7（2026-05-02，轻微规则补充未单独创建快照）；v19.6（2026-05-02，轻微规则补充未单独创建快照）；v19.5（2026-05-02，轻微规则补充未单独创建快照）；v19.4（2026-05-02，轻微规则补充未单独创建快照）；v19.3（2026-05-02，轻微规则补充未单独创建快照）；v19.2（2026-05-02，轻微规则补充未单独创建快照）；v19.1（2026-05-02，轻微规则补充未单独创建快照）；v19（2026-05-02）；v18.2（2026-05-01，轻微规则补充未单独创建快照）；v18.1（2026-05-01，轻微规则补充未单独创建快照）；v18（2026-05-01）；v17（2026-05-01）；v16.7（2026-05-01，轻微规则补充未单独创建快照）；v16.6（2026-04-30，轻微规则补充未单独创建快照）；v16.5（2026-04-30，轻微规则补充未单独创建快照）；v16.4（2026-04-30，轻微规则补充未单独创建快照）；v16.3（2026-04-30，轻微规则补充未单独创建快照）；v16.2（2026-04-27）；v16.1（2026-04-27）；v16（2026-04-27，轻微规则补充未单独创建快照）。
...
> 历史版本由 git log 提供;本仓库不再保留 `skill-flow-{YYYYMMDD}-v{N}.md` 文件式快照(v21.1 起反转,既有快照已删除)。需要某历史版本时执行 `git log --follow docs/skill-flow.md` 找 commit,然后 `git show <sha>:docs/skill-flow.md` 输出。、`docs/skill-flow-20260501-v18.md`（v18）、`docs/skill-flow-20260501-v17.md`（v17）、`docs/skill-flow-20260427-v15.md`（v15）、`docs/skill-flow-20260427-v14.md`（v14）、`docs/skill-flow-20260427-v13.md`（v13）、`docs/skill-flow-20260427-v12.md`（v12）、`docs/skill-flow-20260427-v11.md`（v11）、`docs/skill-flow-20260427-v10.md`（v10）、`docs/skill-flow-20260427-v9.md`（v9）、`docs/skill-flow-20260425-v8.md`（v8）、`docs/skill-flow-20260422-v7.md`（v7）、`docs/skill-flow-20260416-v6.md`（v6）、`docs/skill-flow-20260410-v5.1.md`（v5.1）、`docs/skill-flow-20260404-v4.md`（v4）、`docs/skill-flow-20260403-v3.md`（v3）、`docs/skill-flow-20260402-v2.md`（v2）
