...
>
> **最后更新：2026-04-22 v7**
> 变更摘要：新增 `daily-work-log`（业务项目每日工作日志：bug / 功能 分类沉淀、同主题合并、一行一条明细、预估工时累计）；`korepos-backend-service` 补充 ACL 防腐蚀层规范（三档 L1/L2/L3 策略 + 已对接接口保护矩阵）。
> 上一版：v6 新增 `project-docs-update`（知识图谱持续维护）、`arch-lint`（Flutter 架构违规检测）；`init-project-docs` 升级为 4 阶段渐进式知识图谱构建。
...
| `daily-work-log` | team-standards | 业务项目源码 Edit/Write 后 / 用户说「记录一下」「更新工作日志」/ 会话结束前：按 🐛 Bug 修复 / ✨ 功能开发 分两区写入 `docs/work-log/{YYYY-MM-DD}.md`，同主题合并、预估工时累计 |
| `korepos-backend-service` | team-standards | 写 korepos / korepos-refund 后端接口时：backend 模板 + BackendInfra 门面 + UI 对接手册 + 八步编写顺序 + ACL 防腐蚀层（L1/L2/L3） |
...
| `daily-work-log` 和 `dev-log` 有什么区别？ | `dev-log` 作用于 **team-standards 插件仓库**（记录 skill/模板/配置变更）；`daily-work-log` 作用于 **业务项目**（记录 bug 修复 / 功能开发的工作日志，按日累计工时）。改 team-standards 走 `dev-log`；改业务代码走 `daily-work-log`；两者同时改就都触发，互不重叠。 |
| `daily-work-log` 怎么避免为同一 bug 写多条？ | 写入前 Step 0 必须 Read 当天日志，提取已有 `[Bx]/[Fx]` 条目标题做合并判定；命中已有条目则在"修改明细"子列表追加一行 + 更新累计工时，而非新建条目。 |
| 工时怎么估？ | 基础值按文件改动规模（1-5 文件 / 6-10 文件 / >10 文件），叠加项按是否写了设计文档 / bug 文档 / skill 改动 / Rust FFI 对齐 / 方案迭代。单条目上限 8h，超过应拆子条目。 |
