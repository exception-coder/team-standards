...
**快速导航：** [核心规则](#1-核心规则) · [接口入口](#2-接口入口指针) · [文件清单](#3-涉及文件清单) · [数据结构](#4-数据结构) · [约束](#5-重要约束与边界) · [依赖](#6-下游依赖调用) · [异常处理](#7-异常处理要点) · [验证命令](#8-验证命令)
...
- 状态恢复规则与视觉复核清单分别落入独立 reference，由主流程按阶段读取，禁止把完整审美长文堆回 `SKILL.md`。
...
| 全路径 | 操作 | 说明 |
|---|---|---|
| `plugins/team-standards/skills/frontend-excellence/SKILL.md` | 修改 | 定义触发、流程、硬门禁和引用路由。 |
| `plugins/team-standards/skills/frontend-excellence/agents/openai.yaml` | 已有 | 定义显示名称、短描述和默认提示。 |
| `plugins/team-standards/skills/frontend-excellence/references/architecture-and-state.md` | 已有 | 定义源码结构、组件边界和状态归属。 |
| `plugins/team-standards/skills/frontend-excellence/references/design-system-and-aesthetics.md` | 修改 | 定义视觉方向、令牌、组件状态和反模式。 |
| `plugins/team-standards/skills/frontend-excellence/references/interaction-and-states.md` | 新增 | 定义工作流状态、恢复路径、控制状态、导航和状态文案。 |
| `plugins/team-standards/skills/frontend-excellence/references/visual-review-checklist.md` | 新增 | 定义真实渲染后的视觉层级与通用生成式 UI 模式复核。 |
| `plugins/team-standards/skills/frontend-excellence/references/quality-gates.md` | 修改 | 定义响应式、可访问性、浏览器与交付检查。 |
| `CLAUDE.md` | 修改 | 同步覆盖范围和资源索引。 |
...
frontend-excellence#recover-state — 保留上下文并为非正常状态提供下一步
frontend-excellence#review-visuals — 在真实渲染中清除无依据的通用生成式 UI 模式
frontend-excellence#verify-experience — 在真实浏览器中验证视口、状态和交互
```

## 4. 数据结构

Skill 不新增运行时数据结构。规则资料按以下五类稳定分区：

```text
architecture-and-state    前端目录、依赖、状态、数据边界
design-system-and-aesthetics    视觉语言、令牌、组件与页面构图
interaction-and-states          工作流状态、恢复路径、导航和状态文案
visual-review-checklist         视觉层级与通用生成式 UI 模式复核
...
- 恢复：非正常状态存在可行下一步时不得形成 dead end。
- 视觉：Card、圆角、阴影、渐变和居中布局必须有层级或交互目的，不按组件名称机械禁用。
