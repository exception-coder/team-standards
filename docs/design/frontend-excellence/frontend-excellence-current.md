# Frontend Excellence Skill 技术设计

本文定义 `team-standards` 插件中的产品级前端工程能力。它在既有架构规范之上补齐视觉设计、设计系统、交互完整性、响应式、可访问性与真实浏览器验收，不重复维护 Feature 分层规则。

**快速导航：** [目标与边界](#1-目标与边界) · [整体架构](#2-整体架构) · [模块职责](#3-模块拆分与职责) · [关键交互](#4-关键交互) · [核心规则](#5-核心规则) · [编码落点](#6-编码落点) · [风险](#8-风险与待确认) · [验证](#9-验证要点)

## 1. 目标与边界

- **要解决的问题**：AI 生成的前端容易停留在可运行的页面拼装，缺少稳定的视觉语言、完整交互状态和可验证的工程质量，并反复套用大圆角卡片、机械居中、装饰性阴影和无恢复路径的状态页。
- **本次目标**：维护 `frontend-excellence` Skill，在创建或大幅修改 Web 前端、页面、组件库、仪表盘和落地页时自动触发，直接交付可维护的组件化源码，并通过反通用模板复核消除未经判断的生成式 UI 模式。
- **不做什么**：不绑定单一框架；不复制 `architecture-ddd-lite-fullstack` 的 Feature 分层规则；不强制安装 Figma；不提供固定品牌模板；不替代产品需求确认。
- **设计结论**：以独立 Skill 编排“理解项目约束 → 建立视觉方向 → 设计系统化实现 → 完整状态实现 → 浏览器验证”，并按需加载架构、美学和验收参考。

## 2. 整体架构

```mermaid
flowchart TD
    subgraph Inputs["设计输入"]
        Brief["需求与内容"]
        Reference["截图或视觉参考"]
        Figma["Figma 可选"]
    end

    subgraph Standards["团队标准插件"]
        Frontend["frontend-excellence 新增"]
        Architecture["architecture-ddd-lite-fullstack 已有"]
        Common["coding-standards-common 已有"]
    end

    subgraph Delivery["交付闭环"]
        Source["组件化前端源码"]
        Browser["真实浏览器验证"]
        Evidence["截图与测试证据"]
    end

    Brief --> Frontend
    Reference --> Frontend
    Figma -.-> Frontend
    Frontend --> Architecture
    Frontend --> Common
    Frontend --> Source
    Source --> Browser
    Browser --> Evidence

    style Architecture stroke-dasharray: 5 5
    style Common stroke-dasharray: 5 5
```

## 3. 模块拆分与职责

### 3.1 Skill 主入口

- **定位**：判断前端任务类型并执行端到端质量工作流。
- **职责**：
  - 识别项目技术栈、既有设计系统和真实产品目标。
  - 路由架构、美学与质量参考资料。
  - 强制浏览器验证和交付证据。
- **上游**：前端创建、重做、页面设计、组件库、仪表盘、落地页等用户请求。
- **下游**：现有架构与编码 Skill、项目源码、浏览器或 Playwright。
- **关键设计点**：Figma 仅是可选输入；缺失时允许用截图、品牌材料或明确设计 brief 推进。

### 3.2 架构与状态参考

- **定位**：提供前端源码结构、状态归属和组件职责的具体准则。
- **职责**：
  - 区分路由编排、业务 Feature、领域实体和共享 UI。
  - 约束服务端状态、客户端状态、表单状态与 URL 状态归属。
  - 防止跨 Feature 内部依赖和页面巨型组件。
- **上游**：Skill 主入口。
- **下游**：项目目录和组件实现。
- **关键设计点**：只补充前端特有决策，Feature 单向依赖的规则源仍是 `architecture-ddd-lite-fullstack`。

### 3.3 设计系统与美学参考

- **定位**：把视觉方向转为可维护的设计令牌和组件规则。
- **职责**：
  - 定义字体、色彩、间距、圆角、阴影、层级和动效语言。
  - 约束组件完整状态和页面视觉层级。
  - 避免模板化渐变、无目的卡片堆叠和随机装饰。
- **上游**：Skill 主入口与设计输入。
- **下游**：tokens、基础组件和页面组合。
- **关键设计点**：视觉选择必须服务内容、品牌和任务，不以“炫技”代替信息层级。

### 3.4 交互与工作流状态参考

- **定位**：把加载、空、错、失效、无权限和校验失败视为当前工作流的一部分。
- **职责**：
  - 维持上下文，以“上下文 → 状态 → 解释 → 下一步”组织受阻页面。
  - 要求可恢复状态提供真实可用的返回、重试、修改、申请权限、联系或替代路径。
  - 约束导航、组件状态和状态文案，避免把状态页做成装饰海报。
- **上游**：页面流程、权限和数据结果。
- **下游**：状态组件、导航和恢复操作。
- **关键设计点**：状态图标只辅助理解，禁止用巨大图标和悬浮 Card 代替上下文与下一步。

### 3.5 视觉复核参考

- **定位**：在真实渲染结果上识别通用生成式 UI 模式和视觉层级缺陷。
- **职责**：
  - 复核 Card、圆角、阴影、色彩、Typography、密度、导航和完整状态。
  - 专项检查机械居中、大白卡、万能 Empty State、无意义渐变和全局胶囊化。
  - 要求删除不改善层级、可读性、交互或上下文的视觉元素。
- **上游**：浏览器中的完整页面。
- **下游**：视觉迭代与交付证据。
- **关键设计点**：反模式不是按控件名称一刀切，而是要求每个视觉手段都有产品目的。

### 3.6 质量门禁参考

- **定位**：定义可执行的完成标准。
- **职责**：
  - 覆盖响应式、键盘操作、焦点、对比度和减少动效。
  - 验证加载、空、错误、成功、禁用等状态。
  - 使用真实浏览器检查关键视口和交互路径。
- **上游**：已实现的前端源码。
- **下游**：测试、截图、构建和最终交付说明。
- **关键设计点**：不能仅以编译成功作为前端完成标准。

## 4. 关键交互

### 4.1 从需求到实现

触发：用户要求创建或显著修改前端体验。

```mermaid
sequenceDiagram
    autonumber
    box rgb(217, 226, 246) 输入与分析
        participant U as 用户需求
        participant F as Frontend Skill
    end
    box rgb(212, 237, 218) 规范与实现
        participant A as 架构规范
        participant S as 项目源码
    end

    U->>F: 提供目标与参考
    F->>S: 读取技术栈和既有设计系统
    F->>A: 确认分层与依赖边界
    A-->>F: 返回适用约束
    F->>S: 建立令牌和组件化实现
    S-->>F: 返回可运行页面
```

### 4.2 从实现到视觉验收

触发：页面已能运行，准备完成交付。

```mermaid
sequenceDiagram
    autonumber
    box rgb(212, 237, 218) 本地实现
        participant S as 前端源码
        participant T as 自动化测试
    end
    box rgb(248, 215, 218) 真实环境
        participant B as 浏览器
        participant R as 参考依据
    end

    S->>T: 运行构建和关键测试
    T-->>S: 返回验证结果
    S->>B: 打开关键页面和视口
    B->>R: 对照布局与交互
    R-->>B: 标记差异
    B-->>S: 反馈需要修正的问题
    S->>B: 迭代并复验
```

### 4.3 缺少设计稿时的降级路径

触发：没有 Figma 或完整视觉稿，但用户要求直接产出高质量源码。

```mermaid
sequenceDiagram
    autonumber
    box rgb(217, 226, 246) 方向建立
        participant F as Frontend Skill
        participant I as 可用输入
    end
    box rgb(212, 237, 218) 方案确认
        participant D as 设计令牌
        participant P as 页面原型
    end

    F->>I: 提取品牌、受众、内容和参考
    I-->>F: 返回可用约束
    F->>D: 定义单一视觉方向
    D->>P: 生成代表性页面与状态
    P-->>F: 返回浏览器可验证原型
```

## 5. 核心规则

| 规则 | 说明 |
|---|---|
| 先读项目再选栈 | 已有项目复用其框架、组件库、令牌和数据模式，不平行创建第二套体系。 |
| 设计令牌优先 | 颜色、字体、间距、圆角、阴影、层级和动效必须由语义令牌表达。 |
| 组件状态完整 | 交互组件至少考虑默认、悬停、焦点、激活、禁用、加载和错误状态。 |
| 状态必须可恢复 | 空、错、失效、无权限和校验状态保留上下文，并在存在可行路径时提供下一步。 |
| 层级先于容器 | 优先用留白、对齐、排版、分隔和色调建立结构，Card 与阴影必须表达真实边界或层级。 |
| 反通用模板复核 | 浏览器首轮完整渲染后检查大白卡、机械居中、巨大状态图标、过度圆角、渐变和装饰性阴影。 |
| 响应式是布局规则 | 不以简单缩放桌面稿代替移动端信息重排和交互调整。 |
| 可访问性内建 | 使用语义结构、键盘路径、可见焦点、合理对比度和减少动效支持。 |
| 视觉验收必做 | 至少验证桌面与移动关键视口；存在参考图时必须对照迭代。 |
| 禁止演示稿式源码 | 不用单文件 HTML、巨型页面组件、随机内联样式或假数据散落作为正式交付。 |

## 6. 编码落点

```text
plugins/team-standards/
└── skills/
    └── frontend-excellence/
        ├── SKILL.md                              [修改] 核心触发与执行工作流
        ├── agents/
        │   └── openai.yaml                      [已有] Codex UI 元数据
        └── references/
            ├── architecture-and-state.md        [已有] 前端结构与状态归属规则
            ├── design-system-and-aesthetics.md  [修改] 设计系统和视觉质量规则
            ├── interaction-and-states.md        [新增] 工作流状态、恢复路径与导航规则
            ├── visual-review-checklist.md       [新增] 反通用模板与视觉层级复核
            └── quality-gates.md                 [修改] 浏览器、可访问性与交付门禁

team-standards/
├── CLAUDE.md                                    [修改] 注册触发、索引和链路
├── AGENTS.md                                    [生成] 同步 Codex 入口
├── README.md                                    [修改] 登记新 Skill
├── README_en.md                                 [修改] 同步英文能力说明
├── CHANGELOG.md                                 [修改] 记录新增能力
├── docs/
│   ├── skill-flow.md                            [修改] 接入工作流
│   ├── skill-triggers.md                        [修改] 处理触发重叠
│   ├── skill-dependencies.md                    [修改] 登记依赖关系
│   └── dev-log/2026-08-13.md                    [新增] 记录决策背景
└── plugins/team-standards/
    ├── .codex-plugin/plugin.json                [修改] Minor 版本升级
    └── .claude-plugin manifests                 [修改] 同步版本
```

### 调用关系说明

- 前端需求 → `change-readiness`（含代码定位）→ `architecture-ddd-lite-fullstack` → `frontend-excellence` → `coding-standards-common` → 实现与浏览器验证。

## 7. 数据与依赖变更

| 类型 | 是否变化 | 说明 |
|---|---|---|
| 数据库表、字段、索引 | 无 | 纯插件规则能力。 |
| DTO、VO、枚举 | 无 | 不修改业务契约。 |
| 下游接口、外部依赖 | 无强制变化 | Figma、ImageGen、浏览器和 Playwright 均按环境能力选用。 |
| 缓存、消息、锁、事务 | 无 | 不适用。 |

## 8. 风险与待确认

| 风险 | 影响 | 处理方式 |
|---|---|---|
| 与架构 Skill 重复 | 规则漂移、上下文膨胀 | 新 Skill 只维护前端体验与验收，分层规则引用现有 Skill。 |
| 将某个框架写成唯一答案 | 破坏已有项目约束 | 先识别项目；只有新项目且用户未指定时才给推荐默认值。 |
| 视觉规则过度主观 | 产出风格不稳定 | 要求明确内容层级、受众、品牌线索和参考依据，并用浏览器结果校验。 |
| 反模式规则变成风格禁令 | 为避开 AI 感而机械禁用 Card、圆角或阴影 | 只否定无产品目的的视觉手段，允许表达独立对象、交互边界和真实层级。 |
| 强制外部插件 | 离线或未安装环境无法执行 | Figma 仅增强输入，不作为完成前置条件。 |
| 验收流程过重 | 小范围样式修复效率降低 | 按改动风险选择关键视口和路径，不要求全站截图。 |

## 9. 验证要点

- **正常路径**：新前端项目、现有页面重做、组件库建设均能正确触发并加载相关参考。
- **异常路径**：没有 Figma、没有截图或浏览器不可用时，Skill 能说明降级验证与残余风险。
- **状态路径**：空、错、失效、无权限和校验失败能够保留上下文并提供可信恢复路径。
- **视觉复核**：首轮完整渲染后能识别并修正无依据的大白卡、机械居中、巨大状态图标和装饰性效果。
- **边界条件**：纯后端、单行文案、无视觉影响的逻辑修复不得误触发。
- **回归范围**：Skill 审计、交叉引用、AGENTS 同步、三处版本同步、Hook 测试和插件安装冒烟测试。

## 10. 业务图表与管理看板分层（4.1.0）

本次在前端设计职责域新增独立 business-visualization-advisor，复用已有个人技能的语义与选图规则。管理看板是可独立提出的用户意图，且普通页面无需读取指标规则，因此保持可独立发现的 Skill，而不是隐藏在 frontend-excellence 的参考文件中。

| 层次 | 唯一职责 | 交接信息 |
|---|---|---|
| 业务事实与语义 | backend-evidence / business-logic-orientation 及项目事实源 | 状态、指标口径、聚合、权限、历史证据 |
| 信息设计 | business-visualization-advisor | 业务问题、洞察优先级、选图、Dashboard Spec、指标与明细一致性 |
| 视觉系统 | design-system | 既有 tokens、状态色、组件、明确视觉偏好 |
| 前端实现 | frontend-excellence | 布局、响应式、完整交互、真实浏览器验收 |
| 交付 | delivery-verification | 当前实现的真实验证证据 |

前端入口在业务图表/看板条件下读取专项 Skill；图表评审也能独立触发。管理看板按“目标与结果—差距与风险—原因分解—行动明细”设计，目标、比较基线和原因均需业务证据；不内置公司 KPI、不强制安装图表库、不新增运行时推荐引擎或 Hook。

实现坐标为新 Skill 目录、两个前端 Skill 的条件路由、Agent 入口、能力目录及三处发布元数据。保持 4.0.0 已有调用名，新增能力以 4.1.0 发布。验证包含结构/引用/版本、安装烟测、正常/不触发/证据缺失场景审视。当前仓库未启用 OpenSpec，本节为本次唯一设计依据。

### 4.1.1 选图决策与可感知改进

针对明确要求看板优化却仅修改布局的问题，在 advisor 内增加实施前候选清单门槛：逐项比较并选择采用图表、保留数字卡或不画图；未改变图表需编码前说明理由，不新增审批。真实业务页无法访问时提供授权范围内的替代预览并声明验证边界。交付区分信息架构、图表化与交互改造。规则归 advisor，字段示例归其 Dashboard Spec reference，不复制到视觉系统或新增 Hook。

### 4.1.2 业务目的与反浅层完成

在 advisor 的已有证据入口、候选门槛和交付章节原位强化：实施前明确受众、发现目标、行动和首屏问题；核心区域映射目的、指标、展示和行动；统计总览/看板优化无需点名技能即可进入选图门槛。完成按业务目的反向验证，至少证明决策、洞察、比较、定位或下钻能力之一改善。缺行动的图降为辅助，只读决策场景需明确具体决策路径；无流程节点允许有依据的不适用。保留合理无图与纯样式边界，不新增审批或机械 Hook。
